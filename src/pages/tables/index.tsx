...
const mockTables: Table[] = [
  { id: 1, seats: 4, status: "Occupied", waiter: "Sarah Johnson", orderValue: 45.5, timeOccupied: "1h 15m" },
  { id: 2, seats: 2, status: "Free", waiter: null, orderValue: 0, timeOccupied: null },
  { id: 3, seats: 6, status: "Reserved", waiter: "Mike Chen", orderValue: 0, timeOccupied: null },
  { id: 4, seats: 4, status: "Needs Cleaning", waiter: null, orderValue: 0, timeOccupied: null },
  { id: 5, seats: 2, status: "Occupied", waiter: "Sarah Johnson", orderValue: 32.75, timeOccupied: "45m" },
  { id: 6, seats: 8, status: "Free", waiter: null, orderValue: 0, timeOccupied: null },
  { id: 7, seats: 4, status: "Occupied", waiter: "Alex Thompson", orderValue: 67.25, timeOccupied: "2h 5m" },
  { id: 8, seats: 2, status: "Reserved", waiter: "Mike Chen", orderValue: 0, timeOccupied: null },
  { id: 9, seats: 4, status: "Free", waiter: null, orderValue: 0, timeOccupied: null },
  { id: 10, seats: 6, status: "Occupied", waiter: "Sarah Johnson", orderValue: 89.25, timeOccupied: "30m" },
  { id: 11, seats: 2, status: "Free", waiter: null, orderValue: 0, timeOccupied: null },
  { id: 12, seats: 4, status: "Needs Cleaning", waiter: null, orderValue: 0, timeOccupied: null },
];
...
const updateTableStatus = (tableId: number, newStatus: string) => {
  ...
  const assignWaiter = (tableId: number, waiterName: string) => {
    ...
    if (selectedTable && selectedTable.id === tableId) {
      setSelectedTable(prev => prev ? { ...prev, waiter: waiterName } : null);
    }
  };
  ...
  return (
    <div className="p-5 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Monitor table status and manage assignments</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl">{occupiedTables}/{tables.length}</div>
            <p className="text-xs text-muted-foreground">{Math.round((occupiedTables / tables.length) * 100)}% occupied</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl">{tables.filter(t => t.status === 'Free').length}</div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From occupied tables</p>
          </CardContent>
        </Card>
            ...
            {tables.map((table) => (
              <Dialog key={table.id}>
                <DialogTrigger asChild>
                  ...
                  >
                    <div className="text-center">
                      ...
                      <Badge 
                        variant={statusBadgeColors[table.status as keyof typeof statusBadgeColors]} 
                ...
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>Table {selectedTable?.id} Management</DialogTitle>
                    <DialogDescription>
                    ...
                    <div className="space-y-6 mt-4">
                      <div className="grid grid-cols-2 gap-4 text-center">
                        ...
                        <div>
                          ...
                          <Badge variant={statusBadgeColors[selectedTable.status as keyof typeof statusBadgeColors]} className="mt-1">
                      ...
                      {selectedTable.timeOccupied && (
                        <div className="text-center">
                          ...
                          <p className="text-lg">{selectedTable.timeOccupied}</p>
                        </div>
                      )}

                      {selectedTable.orderValue > 0 && (
                        <div className="text-center">
                          <p className="text-sm text-muted-foreground">Current Order Value</p>
                          <p className="text-lg">${selectedTable.orderValue.toFixed(2)}</p>
                        </div>
                      )}
                      ...
                      <div>
                        <label className="text-sm text-muted-foreground">Assign Waiter</label>
                        <Select 
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button 
                          ...
                          onClick={() => console.log('View current order for table', selectedTable.id)}
                        >
                          View Order
                        </Button>
                      </div>
