...
import { StaffShiftWidget } from "./_component/ShiftClockinWidget";

interface WaiterAssignmentViewProps {
  assignment: Assignment | null | undefined;
  onRefresh: () => void;
  isLoading?: boolean; // Accept an external loading prop if parent manages fetching
}
...
export const WaiterAssignmentView = ({
  ...
  // Sync internal state when assignment prop settles
  useEffect(() => {
    setLoading(false);
  }, [assignment]);

  const isRefreshing = loading || externalLoading;
  console.log({externalLoading,loading})

  return (
    <div className="space-y-6">
      {/* Shift Widget is placed outside conditional checks so staff can always manage shift status */}
      <StaffShiftWidget />
      ...
      {isRefreshing ? (
        <div className="flex flex-col items-center justify-center rounded-[2.5rem] bg-gray-50/50 p-12 text-center ring-1 ring-gray-100">
          <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-3xl bg-white shadow-sm ring-1 ring-gray-100">
            <RefreshCcw size={32} strokeWidth={1.2} className="text-gray-300 animate-spin" />
          </div>
          <p className="text-[14px] font-medium text-gray-400">Loading assignment details...</p>
        </div>
      ) : !assignment ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-[2.5rem] bg-gray-50/50 p-12 text-center ring-1 ring-gray-100">
          ...
          <button className="mt-4 text-[13px] font-bold text-blue-500">
          ...
          <button
            ...
            className={cn(
              "group mt-6 flex items-center gap-2 rounded-xl px-4 py-2 text-[13px] font-bold tracking-tight transition-all active:scale-95",
