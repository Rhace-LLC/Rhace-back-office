...
const getStatusBadgeProps = (status: Reservation['status']) => {
    switch (status) {
        case "confirmed":
            ...
            return { className: "bg-indigo-50 text-indigo-600 border-indigo-200 hover:bg-indigo-100 font-medium", variant: "default" };
        case "confirmed":
            // Primary action/in-progress color (e.g., blue/teal)
            return { className: "bg-teal-50 text-teal-600 border-teal-200 hover:bg-teal-100 font-medium", variant: "default" };
        case "cancelled":
            // Destructive color (red)
            return { className: "bg-red-50 text-red-600 border-red-200 hover:bg-red-100 font-medium", variant: "default" };
        case "pending":
        default:
            // Neutral/waiting color
            return { className: "bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100 font-medium", variant: "default" };
    }
};
        ...
        <Table className="w-full">
            <TableHeader className="bg-gray-50 border-b">
                <TableRow className="hover:bg-gray-50">
                    <TableHead className="w-[100px] text-xs font-semibold text-gray-500 uppercase tracking-wider"><Hash className="inline h-3 w-3 mr-1" /> Ref</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Customer</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap"><Clock className="inline h-3 w-3 mr-1" /> Time</TableHead>
                    <TableHead className="text-xs font-semibold text-gray-500 uppercase tracking-wider"><Users className="inline h-3 w-3 mr-1" /> Guests</TableHead>
                ...
                {data.length === 0 ? (
                    ...
                    data.map((r) => (
                        ...
                        >
                            ...
                            <TableCell>
                                <Badge
                                    className={getStatusBadgeProps(r.status).className + " capitalize text-xs px-2.5 py-1 rounded-full"}
                                >
                                    {r.status}
                                </Badge>
                            </TableCell>
